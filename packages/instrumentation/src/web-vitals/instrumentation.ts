...
export class WebVitalsInstrumentation extends InstrumentationBase<WebVitalsInstrumentationConfig> {
  ...
  override enable(): void {
    ...
    // Each registration is isolated: a vital whose underlying observer type is
    // unsupported by the browser (CLS uses LayoutShift, INP uses EventTiming)
    // throws synchronously from the web-vitals library. Catch per-vital so one
    // unsupported entry type cannot quarantine the others. CLS is only
    ...
    // Set the guard only after all attempts complete so a throw above does not
    // lock us into "already registered" on a future enable() retry.
